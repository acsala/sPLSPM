      ...
      for (l in 1:length(lambdas)){
        ...
        for (nz in 1:length(non_zeros)){
          ...
          for (i in 1:nr_subsets){
            ...
            # sum(apply(X.train,2,sd)==0)
            # sum(apply(X.test,2,sd)==0)

            #first throw away Xtest columns sd=0 from both set
            X.train   =   X.train[,apply(X.test,2,sd)>0]
            X.test    =   X.test[,apply(X.test,2,sd)>0]

            #then throw away Xtrain columns from both set
            ...
            # sum(apply(Y.train,2,sd)==0)
            # sum(apply(Y.test,2,sd)==0)
