                          ...
                          nonzero, lambda, warning_non_convergence = TRUE, alpha)
{
  ...
  if(anyNA(apply(X %*% ODM, 2, sd))){
    print("SD=0 Columns in datasets!")
...
#    Z = Z %*% diag(1/(apply(Z,2,sd)*sdv), lvs, lvs)  # scaling Z
    ...
    for (j in 1:lvs)
    {
      if (.subset2(specs$modes,j) == "A")
        #W[blockinds==j,j] <- (1/nrow(X)) * (Z[,j] %*% X[,blockinds==j])
        W[blockinds==j,j] <- (1/nrow(X)) * (crossprod(X[,blockinds==j], Z[,j]))
      if (.subset2(specs$modes,j) == "B")
